...
| HP OCR 与写入 | status panel OCR 的 `hp_text` / `hp_percentage` 已汇总为 `player_hp_current/max` 与 `opponent_hp_percent`，并可继续同步 BattleSession | 55% | `ChineseOcrSideRecognizer._extract_hp`, `RecognitionPipeline._parse_hp_pair`, `BattleSessionStore.update_player_hp`, `test_recognition_pipeline_enhanced.py`, `test_battle_session_store.py` | mock/status-panel 链路已闭合；仍需真实 OCR 截图端到端验收，独立 `player_hp_text`/`opponent_hp_bar` ROI 还没有专门 OCR enrich |
