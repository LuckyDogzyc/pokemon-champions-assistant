...
|---|---|---:|---|---|
| BattleSession 数据模型 | 模型和 store 存在，已补第一批生命周期测试，正在成为主数据源 | 60% | `backend/app/schemas/battle_session.py`, `backend/app/services/battle_session_store.py`, `backend/tests/test_battle_session_store.py` | 已覆盖 slot 顺序、active、HP、moves、status、final_result/LOG 保留；仍需打通旧 `BattleStateStore.move_log` 自动同步与更多 API 端到端 |
| 每局独立 JSON，UI 从对象读取 | 首页尝试读取 `battle_session`，但契约不完整 | 40% | `frontend/app/page.tsx` 使用 `(state as any)?.battle_session` | `RecognitionState` 类型没有 `battle_session`；用 `any` 绕过类型；API/前端契约漂移 |
...
| 状态异常识别 | Store 可从 ROI payload 写入 BattleSession；真实 OCR → payload 仍需验收 | 60% | `ChineseOcrSideRecognizer._extract_status_abnormality`, `BattleStateStore._parse_status`, `BattleSessionStore.update_statuses_from_roi_payloads` | 状态 LOG 仍未进入 `BattleSession.log`；需要标准状态枚举映射和真实 OCR 端到端测试 |
| 对战 LOG 自动生成 | 旧 `BattleStateStore` 有雏形，BattleSession LOG 未闭环 | 30% | `BattleStateStore._make_log`, `BattleSessionStore.append_log` | `BattleSession.log` 不自动接收旧 `move_log`；LOG 保留已有 store 级测试，但自动生成链路仍无测试 |
| 结算清数据但保留 LOG | Store 级实现为“结算 freeze；下一局 TEAM_SELECT 清战斗数据并保留 LOG” | 55% | `BattleSessionStore.sync_from_recognition`, `test_battle_session_store.py` | 行为已固定为下一局清；仍需 API/UI 端到端测试；旧 `BattleStateStore` final_result reset 仍可能丢 log |
| 手动修正 API | 旧 recognition override 会同步 BattleSession active；PRD API 不存在 | 45% | `POST /api/recognition/override`, `test_manual_override_api.py` | PRD 写的 `/api/battle-session/manual-override` 未实现；还不支持 team slot/item/HP/move/status 修正 |
| `/api/battle-session/status` | 未实现 | 0% | 实际为 `GET /api/battle/session` | API 路径与 PRD 不一致，需要决定补实现还是改 PRD |
