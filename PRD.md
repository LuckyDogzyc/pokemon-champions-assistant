...
| 当前出战宝可梦 | Store 层已能在无选人阶段时直接补齐双方 active 基础数据；HP、状态、技能也可写入 active | 60% | `set_player_active`, `set_opponent_active`, `_mon_from_name`, `test_battle_session_store.py` | active Store 闭环增强；仍需真实 OCR/API 连续帧验证，并收敛 BattleStateStore/BattleSessionStore 双轨 |
| HP OCR 与写入 | status panel 与独立 `player_hp_text` / `opponent_hp_bar` ROI 的 OCR 结果都会汇总为 `player_hp_current/max` 与 `opponent_hp_percent`，并可继续同步 BattleSession | 60% | `ChineseOcrSideRecognizer._extract_hp`, `_recognize_hp_roi`, `RecognitionPipeline._parse_hp_pair`, `BattleSessionStore.update_player_hp`, `test_recognition_pipeline_enhanced.py`, `test_battle_session_store.py` | mock/OCR payload 链路已闭合；仍需真实 OCR 截图端到端验收和更稳的中文/噪声样本覆盖 |
| 技能 4 招 + PP | Store 可写入技能和 PP；`MoveListRecognizer` 已改为 moves_index 专用 matcher，可模糊匹配技能名并解析 PP | 60% | `MoveListRecognizer`, `MoveNameMatcher`, `BattleSessionStore.set_player_moves`, `test_move_list_recognizer.py`, `test_battle_session_store.py` | OCR 截图真实端到端仍需验收；中文技能别名数据仍不足；move slot 与整块 move_list 的语义还需继续收敛 |
...
| 手动修正 API | PRD 路径 `/api/battle-session/manual-override` 已实现，可同步 active 名称、我方 HP、对方 HP 百分比和状态；旧 recognition override 仍保留 | 70% | `POST /api/battle-session/manual-override`, `POST /api/recognition/override`, `test_battle_session_api.py`, `test_manual_override_api.py` | 已支持 active + HP/status 修正；仍不支持 team slot/item/move 的结构化修正 |
